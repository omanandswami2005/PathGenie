...
const PROFILE_URL = "http://localhost:5173/profile";
...
const Login = () => {
  ...
  const handleSocialSignIn = async (provider, options = {}) => {
    try {
    const response =  await signIn.social({
        provider,
        callbackURL: DASHBOARD_URL,
        newUserCallbackURL: PROFILE_URL,
        ...options,
      });
      if (response.error) {
      ...
      setErrors((prev) => ({ ...prev, general: `Failed to sign in with ${provider}. Please try again.` }));
    }
  };
